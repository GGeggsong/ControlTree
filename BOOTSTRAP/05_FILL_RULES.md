...
## R11 · 使用者偏好（`user_preferences`）

- 若 `control-tree.config.yml` 含 `user_preferences`（見範例檔與 `01_INTERVIEW.md`），後續協作與訪談節奏**應優先尊重**其中欄位，讓同一使用者在不同 AI／不同對話間維持一致習慣。
- **不得**以偏好為由違反 R3（狀態詞）、R4（合約保護）、R6（同步紀錄）等硬性規則；遇衝突時**硬性規則優先**，並可在 `SYNC_HISTORY.md` 註明「因合約／驗證需求仍須追問」。
- `user_preferences` 可由使用者手改；AI 在訪談收尾或使用者明文要求時，應把當下約定寫回 yml，避免下次重講。

## R12 · 共用文件單一位置原則（`_SHARED/`）
