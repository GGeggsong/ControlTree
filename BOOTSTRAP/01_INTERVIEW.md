...
> 給 AI：預設你必須**用使用者的語言**（預設繁體中文）**逐題**提問；**每題要等使用者回答後再問下一題**，不要一次問完。**禁止**在單一則訊息裡把 Q1～Q6（或尚未作答的多題）整包貼給使用者——使用者請你「模擬訪談」時也一樣，須與實際建樹**相同節奏**，否則沒有模擬價值。
>
> **可改為「表單一次填完」模式（`batch_form`）** 若下列**任一**成立：
>
> 1. 使用者**明文**說「請把六題整份列出來我自己填／列印」或同義句（一次給六題表單）。
> 2. 本次對話已能讀到中控樹根的 `control-tree.config.yml`，且其中 `user_preferences.interview_pace` 為 `batch_form`。
> 3. 使用者開場即表明習慣**一次填完、不要逐題問**（視同選 `batch_form`；建樹寫入 yml 時**必須**寫入 `user_preferences`）。
>
> `batch_form` 時：盡早貼出 **Q1～Q6 表單版**（題號＋欄位／留白），讓使用者**一次回覆**；缺漏再以**最少回合**補問。無論逐題或表單，**建樹前仍須有一次復述彙整**請使用者確認（見「訪談結束後」）。逐題模式的價值在降低卡關；表單模式不必反覆問「要不要下一題」類冗語。
>
> 回答結束後，把所有答案彙整成 `control-tree.config.yml`（範例見根目錄）。

---

## 訪談規則

- **預設逐題**：不要 batching。**每一則**發給使用者的訊息**最多一題**（Q4 的（甲）（乙）視為**同一題**的連續提問，可寫在同一則裡；不要拆成要來回六則才答完一題）。**禁止**在未啟用 `batch_form` 時把多題或未作答的題目整包貼上。若已依上文啟用 `batch_form`，則改為**單則或最少則**表單＋補問。
- **逐題模式**：每題要等使用者回答後再問下一題。**`batch_form` 模式**：等使用者交回整份表單後再統一處理，缺項再以最少回合補問。
- 每題提供**範例答案**（表單模式中可集中附在表單末段），降低使用者卡關機率。
...
3. **同步寫入** `control-tree.config.yml`（建在中控樹根目錄），且**必含** `user_preferences`（至少 `interview_pace`：`step_by_step` 或 `batch_form`；若使用者另有協作風格約定一併寫入，見根目錄範例 yml）。
