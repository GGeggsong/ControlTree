...
我在做一個叫 **projectA** 的產品。一邊是 BLE 產品的韌體（TI MCU、CCS 工程，跑在 Windows），另一邊是 iOS App（Swift、Xcode，跑在 Mac），未來還會接 Android。我在 Mac 上用 Cursor + Claude，在 Windows 上開另一個 Cursor + Codex，偶爾還用 Gemini 釐清概念。
...
所以我做了一棵叫 `projectA_ControlTree` 的中控樹，把所有跨端脈絡寫成 md 檔，讓任何 AI 在任何環境讀完就能上工。用了一陣子之後，我發現這個骨架其實可以脫離韌體／App 的特定情境，**通用化成任何專案都能用**。這就是這篇文章要講的 **Control Tree（中控樹）**。

---

## 2. 核心觀念

骨架三句話：**§2.1** Sync as Files · **§2.2** 核心固定、分支動態、儲存可換 · **§2.5** 任何 AI、任何 OS。**精隨**在 **§2.1.1**：中控樹不替你裁定組織名詞，只把「要分開追、又要能對齊」的東西檔案化。
...
### 2.1.1 精隨 — 樹不替你分類，只幫你對齊

中控樹要解決的**不是**「這到底叫子專案還是獨立專案」這類 taxonomy，而是：

> **凡是需要分開追、卻又要能對齊的 work，能不能有一份共用的檔案化脈絡？**

建樹時你宣告的每一條線（寫進 `05_` 起的 `0X_<SIDE>_SIDE/`；示意圖上常標成 project I / II / III，文件裡也可能叫端別、分支、協作線）——在**你的組織語言**裡可以是：

- 某個大案底下的**子專案**或子交付線；
- 業務上各自獨立、但仍要共用規格或共用檔的**專案**；
- 不同機器、不同 AI、不同工具鏈上的**並行工作**。

這些說法**不衝突**，也**沒有高下**。樹只問兩件事：**你要分幾條線追？有沒有要共用的一份規格（`01_INTERFACE_CONTRACT/`）與一份 `_SHARED/`？**

- **中央**（`00_PROJECT_CONTROL/` 等）→ 彙總、對帳、任務與版本儀表板。
- **各線**（各 `0X_<SIDE>_SIDE/`）→ 那條線自己的現況、交接、測試與筆記。
- **`_SHARED/`** → 全樹都要指過去的 SOP、圖、PDF，**單一位置、不複製**。

若幾條線的脈絡**不該混在同一個儀表板**，就各自建一棵 `<Name>_ControlTree/`（多次 INIT）；若**本來就要同一棵樹裡對帳**，就一棵樹、多條線——兩種都是正確用法，取決於你要不要共用同一份「外腦」。
...
外加一個底線開頭的特殊資料夾 `_SHARED/`，用來放「全樹 AI 都該知道」的共用文件（見 §3）。

「分支」（建樹腳本裡的 Side／端別）是**動態的**，由使用者宣告——名字你愛怎麼叫都行，樹只負責給它一個資料夾與五隻 md。可以是 `firmware / ios / android`，也可以是 `web / cloud / hardware`、`mechanical / factory`，或三個子專案代號；**不必**長得像傳統「韌體 + App」專案。每一條線對應一個 `0X_<SIDE>_SIDE/` 資料夾（**X 從 05 起**：第 1 條線 `05_<SIDE>_SIDE`、第 2 條 `06_<SIDE>_SIDE` … 至 `98` 止），裡面 5 隻檔案：

- `<SIDE>_STATUS.md` — 現況
- `<SIDE>_AI_HANDOFF.md` — 給下一隻 AI 的交接書
- `<SIDE>_CHANGELOG.md` — 版本／變更紀錄
- `<SIDE>_TEST_LOG.md` — 測試紀錄
- `<SIDE>_SKILL_NOTES.md` — 該線的私房知識
...
分支不限韌體／App／雲端：行銷**文案**、硬體機構／試產、視覺與包裝、營運**計畫**與里程碑，甚至三個**子專案代號**，都可以宣告成 `05_` 起的 `<SIDE>_SIDE/`，把狀態與交接寫在該線的五隻檔案裡；不涉及跨線共用契約的敘事則收斂在 `04_PRODUCT_SPEC/`。跨線或對外的介面仍由 `01_INTERFACE_CONTRACT/` 當單一真相；附件進 `_SHARED/`。這樣**軟體、硬體、文案、計畫、多條並行專案線**能在同一棵樹裡被不同 AI 讀寫而不混成單一長聊天（見 §2.1.1）。
...
進行中時，各線可把「這台／這條 build 才會踩到的雷」記在 `0X_<SIDE>_SIDE/<SIDE>_SKILL_NOTES.md`。**封館、釋出、或某一條線結案**時，再把**跨線可重用**的套路、除錯路徑、domain 知識整理進 `03_SKILLS_EXTRACTION/`（例：`DOMAIN_<topic>.md`、`BUG_FIX_<topic>.md`），必要時再匯出成你慣用的 AI Skill 檔或內部 playbook，帶進下一個專案 — 中控樹變成「經驗的停車場」，而不是結案即丟的對話紀錄。

### 2.7 一張圖：中控樹在做什麼（示意）

下面這張**概念圖**和實際目錄不必逐字對應；重點是關係：**一棵 Control Tree 在中間彙總，多條 work 線從外圍連進來，共用檔集中在樹裡。**
...
- **Project A**：這次 INIT 命名的專案（例 `projectA` → `projectA_ControlTree/`）。
- **Project / Side I · II · III**：你在訪談裡宣告的協作線——可以是子專案、可以是獨立專案語意下的三條線、也可以是三台機器上的並行工作；**圖不裁定**。
- **Control Tree**：`00`～`04`、`99` 與 `_SHARED/` 等組成的**同一棵**樹；不是三棵小樹。
- **Shared files**：`_SHARED/`；規格本文仍在 `01` / `04` 的 md，附件用連結指過來。

> 圖說（中英皆可貼在簡報旁）：**One Control Tree — multiple tracks you declare; status rolls up; assets stay in one shared place.**
